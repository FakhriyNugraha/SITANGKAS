...
class SimulationController extends Controller
{
    public function index(): View
    {
        ...
        $categories = CyberCase::where('is_active', true)
            ->selectRaw('category, category_name, COUNT(*) as total')
            ->groupBy('category', 'category_name')
            ->orderBy('category_name')
            ->get();

        return view('user.simulations.index', [
    ...
    public function start(Request $request): RedirectResponse
    {
        $data = $request->validate([
            'mode' => ['required', 'in:category,mixed,recommended'],
            'category' => ['nullable', 'string'],
        ]);

        $setting = SimulationSetting::current();
        $total = $setting->default_case_count;

        $query = CyberCase::where('is_active', true);
        if ($data['mode'] === 'category' && ! empty($data['category'])) {
            $query->where('category', $data['category']);
        }
        if ($data['mode'] === 'recommended') {
            // simple: ambil dari kategori terlemah user di sesi terakhir; jika tidak ada, mixed
            $rec = app(RecommendationService::class);
            $lastSession = SimulationSession::where('user_id', $request->user()->id)
                ->where('status', 'completed')->latest()->first();
            if ($lastSession) {
                $weak = $rec->findWeakCategories($request->user()->id, $lastSession->id);
                if (! empty($weak)) {
                    $query->whereIn('category', array_slice($weak, 0, 3));
                }
            }
        }

        $cases = $setting->randomize_cases
            ? $query->inRandomOrder()->limit($total)->pluck('id')
            : $query->limit($total)->pluck('id');

        if ($cases->isEmpty()) {
            return back()->withErrors(['mode' => 'Tidak ada kasus tersedia untuk pilihan ini.']);
        }

        $session = SimulationSession::create([
            'user_id' => $request->user()->id,
            'mode' => $data['mode'],
            'selected_category' => $data['mode'] === 'category' ? ($data['category'] ?? null) : null,
